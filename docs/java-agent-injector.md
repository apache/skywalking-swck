...
After activing java agent injectoin, if not set the annotations , the injector will use default agent configuration directly as below.
...
| `strategy.skywalking.apache.org/agent.Overlay`    | Whether to overlay the agent configuration. If set true , then you can see next details to configure agent ，if set false, then you can skip [Configure agent](#configure-agent). | `false`                  |
...
| -------------------------------- | ------------------------------------------------------------ | ---------------- |
| `optional.skywalking.apache.org` | Select the optional plugin which need to be moved to the directory(/plugins). User can select serveral optional plugins by seperating from `|` , such as `trace|webflux|cloud-gateway-2.1.x` | not set          |
...
| Annotation key                            | Description                                                  | Annotation value |
| ----------------------------------------- | ------------------------------------------------------------ | ---------------- |
| `optional-reporter.skywalking.apache.org` | Select the optional reporter plugin which need to be moved to the directory(/plugins). User can select serveral optional reporter plugins by seperating from `|` ，such as `kafka` | not set          |
